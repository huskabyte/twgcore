...
public class CommandUnlearn extends CommandBase{
	
	private final List<String> ALIASES = Lists.newArrayList(Reference.MODID, "unlearn");
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 1) {
			...
			sender.sendMessage(new TextComponentString("This command may be used only by a PLAYER! (Soon coming to console)"));
		}else {
			EntityPlayer player = (EntityPlayer) sender;
			for (Entity entity : player.world.loadedEntityList) {
		            if (entity instanceof EntityBondable) {
		            	...
		            	if(entityBondable.getOwner().equals(player)) {
		            		entityBondable.unlearn(Integer.parseInt(args[0]));
		            	}
		            	return;
		            }
			 	}
