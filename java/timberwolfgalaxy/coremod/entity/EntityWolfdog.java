...
public class EntityWolfdog extends EntityBondable{
	...
	protected void registerKnownTricks() {
		if(this.isTamed()) {
			...
			if(PermissionAPI.hasPermission((EntityPlayer) this.getOwner(), "twgcore.bonded.wolfdog.trick2")) {
				this.dataManager.set(KNOWSTRICK2, Byte.valueOf((byte) (b0 | 2)));
			}else {
				this.dataManager.set(KNOWSTRICK2, Byte.valueOf((byte) (b0 & -3)));
			}
			if(PermissionAPI.hasPermission((EntityPlayer) this.getOwner(), "twgcore.bonded.wolfdog.trick3")) {
